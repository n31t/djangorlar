...
from django.db.models import (
    ...
    Case, When, ExpressionWrapper, IntegerField
)
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.limit = 10  # Limit for displaying results
    ...
    def query_2_10(self):
        ...
        queryset = self.CustomUser.objects.order_by('-date_joined')[:20]
        self.print_results(queryset, ['email', 'date_joined'])
    ...
    def query_2_11(self):
        ...
        self.stdout.write(f'Found {len(city_list)} distinct cities:')
        for city in city_list[:self.limit]:
    ...
    def query_2_16(self):
        self.stdout.write(self.style.WARNING('\n[2.16] IT, HR, Sales Departments'))
        queryset = self.CustomUser.objects.filter(
            department__in=['IT', 'HR', 'Sales']
        )
    ...
    def query_2_20(self):
        ...
        self.print_results(queryset, ['email', 'date_joined', 'last_login'])
    ...
    def query_2_30(self):
        ...
        queryset = self.CustomUser.objects.order_by('date_joined')[:10]
        self.print_results(queryset, ['email', 'date_joined'])
    ...
    def query_2_44(self):
        ...
        self.print_results(queryset, ['email', 'date_joined'], annotated=['days_since_joined'])
    ...
    def print_results(self, queryset, fields, annotated=None):
        """Print query results"""
            ...
            for field in fields:
                ...
                if value is not None:
                    if isinstance(value, Decimal):
                        values.append(f'{field}={value:,.2f}')
                    elif hasattr(value, 'strftime'):
                        values.append(f'{field}={value.strftime("%Y-%m-%d %H:%M:%S")}')
                    else:
        ...
        if total > self.limit:
            self.stdout.write(f'  ... and {total - self.limit} more (total: {total})')
        elif total == 0:
